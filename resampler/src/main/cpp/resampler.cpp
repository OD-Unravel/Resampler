...
Java_team_unravel_resampler_NativeLib_stringFromJNI(
        JNIEnv* env,
        jobject /* this */) {
...
Java_team_unravel_resampler_Resampler_close(JNIEnv *env, jobject thiz) {
    auto *resampler = getHandle<oboe::resampler::MultiChannelResampler>(env, thiz);
...
extern "C"
JNIEXPORT void JNICALL
Java_team_unravel_resampler_Resampler_initResampler(JNIEnv *env, jobject thiz, jint source_sample_rate, jint target_sample_rate, jint target_channel_count) {
    g_object = createGlobalReference(env, thiz, thiz);
    auto *resampler = oboe::resampler::MultiChannelResampler::make(target_channel_count, source_sample_rate, target_sample_rate, oboe::resampler::MultiChannelResampler::Quality::Best);
...
Java_team_unravel_resampler_Resampler_processAudioBuffers(JNIEnv *env, jobject thiz, jobject source_buffer,
    ...
    auto *oboeResampler = getHandle<oboe::resampler::MultiChannelResampler>(env, thiz);
    ...
    if (oboeResampler != nullptr && source_channel_count > 0 && target_channel_count > 0) {
        ...
        while (inputFramesLeft > 0) {
            if (oboeResampler->isWriteNeeded()) {
                populateInputBuffer(sourceBuffer, sample_count - inputFramesLeft, resamplerInputBuffer, (int) source_channel_count, (int) target_channel_count);
